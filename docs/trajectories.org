...
=atooms= provides a command line tool to convert between various trajectory formats. The following command will convert a trajectory file produced by [[http://rumd.org][RUMD]] into a simpler xyz format

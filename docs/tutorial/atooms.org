...
As a simple test, we compute the interaction energy between two Lennard-Jones particles
#+begin_src python :exports both :results output :tangle /tmp/test_energy.py
  ...
  # The par_particle option normalizes the energy by the number of particles
  print(system.potential_energy(cache=True, per_particle=True))
  # Compute energy and forces.
  # We access energy and forces stored in the interaction object
  system.compute_interaction("forces")
  print(system.interaction.energy)
  print(system.interaction.forces[0], system.interaction.forces[1])
#+end_src
...
#+begin_src gnuplot :file lammps.png
  ...
  set xtics nomirror
  set ytics nomirror
  plot 'lammps.xyz.thermo' u 1:3 noti w lp lc rgb 'red' pt 7, 2 noti lc rgb 'black'
#+end_src
